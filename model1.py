...
Original file is located at
    https://colab.research.google.com/drive/1AP7cvwmvj89r0tWuEuw74nPiFonl8Ps_
"""
...
'''autisticPath = "/content/drive/MyDrive/train/train/autistic/"
non_autisticPath = '/content/drive/MyDrive/train/train/non_autistic/'

def readData():
    train_data = []
    for imgName in tqdm(os.listdir(autisticPath)):
        path = os.path.join(autisticPath, imgName)
        image = imread(path, True)
        train_data.append([np.array(resize(image, (150, 150))), [1, 0]])

    for imgName in tqdm(os.listdir(non_autisticPath)):
        path = os.path.join(non_autisticPath, imgName)
        image = imread(path, True)
        train_data.append([np.array(resize(image, (150, 150))), [0, 1]])

    shuffle(train_data)
    np.save('/content/drive/MyDrive/training_data.npy', train_data)
    return train_data
  
x=np.array(readData())'''

'''xtrain = [i[0] for i in tqdm(x)]
ytrain = [i[1] for i in tqdm(x)]
...
'''x_train, x_test, y_train, y_test = train_test_split(xtrain, ytrain, test_size=0.1, random_state=1)
...
img = imread('/content/drive/MyDrive/test/test/200.jpg',True)
img = np.array(resize(img,(150,150)))
